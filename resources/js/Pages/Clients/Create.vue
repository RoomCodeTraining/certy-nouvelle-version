...
<template>
    <DashboardLayout>
        <template #header>
            <PageHeader :breadcrumbs="breadcrumbs" title="Nouveau client" />
        </template>

        <div class="min-h-[60vh] flex flex-col w-full max-w-none">
            <form @submit.prevent="form.post(route('clients.store'))" class="rounded-xl border border-slate-200 bg-white p-6 md:p-8 space-y-4 w-full">
                <!-- Type assuré : TAPP / TAPM en premier -->
                ...
                <div class="flex gap-3 pt-4 border-t border-slate-200">
                    ...
                    <Link :href="route('clients.index')" class="px-4 py-2 border border-slate-200 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50">
                        Annuler
                    </Link>
                </div>
            </form>
