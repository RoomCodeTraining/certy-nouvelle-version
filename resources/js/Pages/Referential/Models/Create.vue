...
<template>
    <DashboardLayout>
        <template #header>
            <PageHeader :breadcrumbs="breadcrumbs" title="Nouveau modèle" />
        </template>

        <ReferentialTabs active="models" />

        <div class="min-h-[60vh] flex flex-col w-full max-w-none">
            <form @submit.prevent="form.post(route('referential.models.store'))" class="rounded-xl border border-slate-200 bg-white p-6 md:p-8 space-y-4 w-full">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input v-model="form.name" type="text" required :class="[inputClass, form.errors.name && inputErrorClass]" placeholder="Ex. Clio, 208" />
