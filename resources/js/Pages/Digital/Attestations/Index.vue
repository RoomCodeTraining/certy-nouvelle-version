...
function buildQuery(overrides = {}) {
    ...
    if (f.search != null && String(f.search).trim() !== '') q.search = String(f.search).trim();
...
const viewUrl = computed(() => (viewReference.value ? route('digital.attestations.view', viewReference.value) : null));
/** URL de téléchargement : priorité à download_link de l'API, sinon route locale */
const downloadUrl = computed(() => {
    const link = viewRow.value?.download_link;
    if (link) return link;
    return viewReference.value ? route('digital.attestations.download', viewReference.value) : null;
});
...
watch(showViewModal, (open) => {
    if (open) {
        ...
        const onEscape = (e) => { if (e.key === 'Escape') closeViewModal(); };
...
function producerEmail(row) {
    const u = producerUser(row) ?? row.user ?? row.editor ?? row.created_by ?? row.printed_by ?? row.edited_by;
    if (typeof u === 'object' && u?.email) return u.email;
    return row.producer_email ?? row.editor_email ?? row.printed_by_email ?? row.user_email ?? '—';
}
        ...
        <div class="rounded-xl border border-slate-200 bg-white overflow-hidden">
            <div class="px-4 py-3 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3 bg-slate-50">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">Données issues de la plateforme ASACI</p>
                </div>
                <form
            ...
            <div class="px-4 py-3 border-t border-slate-200 flex flex-wrap items-center justify-between gap-3 bg-slate-50">
                <div class="flex items-center gap-4">
                    ...
                    <select
                        :value="perPage"
                        @change="(e) => router.visit(urlForPerPage(Number(e.target.value)))"
            ...
            >
                ...
                >
                    <div class="bg-white rounded-xl border border-slate-200 flex flex-col max-w-4xl w-full max-h-[90vh] overflow-hidden">
                        ...
                        <div class="flex-1 min-h-0 flex items-center justify-center bg-slate-100 p-2 relative overflow-auto">
                            <div
                                v-if="viewLoading"
                            ...
                            >
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <span class="text-sm text-slate-500">Chargement de l'attestation…</span>
                                </div>
                            </div>
                            <div
                                v-else-if="viewError"
                            ...
                            >
                                <p class="text-sm text-slate-600 text-center">
                                    Impossible d'afficher l'attestation. Vous pouvez la télécharger.
                                </p>
                                <a
                                    v-if="downloadUrl"
                                    :href="downloadUrl"
