...
<template>
    <DashboardLayout>
        <template #header>
            <PageHeader :breadcrumbs="breadcrumbs" title="Créer un bureau" />
        </template>

        <div class="min-h-[60vh] flex flex-col">
            <p class="text-sm text-slate-600 mb-4">
                Créer un bureau sur la plateforme ASACI (service externe).
            </p>
            ...
            <div class="flex-1 w-full max-w-none">
                <form @submit.prevent="form.post(route('digital.bureaux.store'))" class="rounded-xl border border-slate-200 bg-white p-6 md:p-8 space-y-4 w-full">
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Nom *</label>
                    <input v-model="form.name" type="text" required :class="[inputClass, form.errors.name && inputErrorClass]" placeholder="Nom du bureau" />
                    <p v-if="form.errors.name" class="mt-1 text-sm text-red-600">{{ form.errors.name }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Code</label>
                    <input v-model="form.code" type="text" :class="[inputClass, form.errors.code && inputErrorClass]" placeholder="Code bureau" />
